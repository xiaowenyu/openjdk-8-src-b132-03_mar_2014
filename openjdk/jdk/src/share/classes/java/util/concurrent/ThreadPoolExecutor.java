...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    //所有任务已终止，workCount为0，处于TIDYING状态的线程将调用钩子线程terminated()
     ...
     * for work.
     */
    //核心线程是否允许超时等待，false为空闲时一直存活，true为超时杀掉
        ...
        implements Runnable
    {
         ...
         */
        //指定传参firstTask和从threadFactory创建线程
        ...
        //实现Runnable的run方法，线程真正运行的方法runworker
        public void run() {
            runWorker(this);
        }
    ...
    final void tryTerminate() {
        ...
        for (;;) {
            int c = ctl.get();
            //线程池状态为run 或者 状态是TIDYING或者TERMINED
            //或者 线程池状态为SHUTDOWN 且工作队列不为空
            //直接返回
            if (isRunning(c) ||
            ...
            mainLock.lock();
            try {
                //cas ctl 状态为TIDYING workerCount为0
                if (ctl.compareAndSet(c, ctlOf(TIDYING, 0))) {
                    try {
                        //终止线程
                        terminated();
                    } finally {
                        //cas ctl 状态为TERMINATED workerCount为0
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                //wc >= CAPACITY 大于最大容量(2^29 -1)
                //判断线程数量是否越界（分别：核心线程数，最大线程数）
                if (wc >= CAPACITY ||
                ...
                if (compareAndIncrementWorkerCount(c))
                    break retry;
                //失败则重新判断线程池状态，改变了就直接跳到retry，否则继续自旋
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            // Check if queue empty only if necessary.
            //如果线程池状态为SHUTDOWN以上 且 （STOP以上或队列为空）
            //意思是线程池STOP状态时会将队列中的任务跑完
    ...
    final void runWorker(Worker w) {
        ...
        try {
            ...
            while (task != null || (task = getTask()) != null) {
                ...
                //如果当前线程池是stop级别以上，且该线程不是中断的，那么将其中断
    ...
    public void execute(Runnable command) {
         ...
         */
        int c = ctl.get();
        //如果ctl中存放的workCount(线程数) 小于 核心线程数
        ...
        //大于核心线程数且如果线程池正在运行，则加入到队列中
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            //重新检查ctl的状态，不是运行时状态，且在队列中移除command成功
        ...
        //大于core，阻塞队列满时，尝试在小于maximumPoolSize范围运行它
     ...
     * @throws SecurityException {@inheritDoc}
     */
    //SHUTDOWN线程池，并清空队列，中断正在跑的线程，即丢失队列里面的任务
