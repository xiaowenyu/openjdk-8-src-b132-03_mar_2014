 ...
 */
//并发的HashMap
    //参考https://yq.aliyun.com/articles/36781
    //2.~https://blog.csdn.net/u010723709/article/details/48007881
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * @since 1.8
     */
    // 返回Mappings中的元素个数，官方建议用来替代size。此方法返回的是一个估计值；
    // 如果sumCount时有线程插入或删除，实际数量是和mappingCount不同的。since 1.8
     ...
     */
    //连接两个table
        // 并不是我们传统的包含key-value的节点，只是一个标志节点，并且指向nextTable，
    // 提供find方法而已。
...
// RESIZE_STAMP_BITS=16,(左侧0的个数)|(2^15)
     ...
     */
    // 协助扩容方法。多线程下，当前线程检测到其他线程正进行扩容操作，
    // 则协助其一起扩容；（只有这种情况会被调用）从某种程度上说，其“优先级”很高，
    ...
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {
        ...
        for (int i = 0, bound = 0;;) {
            ...
            //该节点为空，则插入ForwardingNode
                ...
                //遍历到ForwardingNode节点，说明此节点被处理过了，
                // 直接跳过。这是控制并发扩容的核心
            else if ((fh = f.hash) == MOVED)
...
// HashMap的TreeNode继承至LinkedHashMap.Entry；而这里继承至自己实现的Node，将带有next指针，便于treebin访问。
    ...
    static final class TreeNode<K,V> extends Node<K,V> {
        ...
        // 和HashMap相比，这里的TreeNode相当简洁；ConcurrentHashMap链表转树时，
        // 并不会直接转，正如注释（Nodes for use in TreeBins）所说，
        // 只是把这些节点包装成TreeNode放到TreeBin中，再由TreeBin来转化红黑树
    }
    ...
    static final class MapReduceMappingsToIntTask<K,V>
        ...
        //在源码的6277行到最后，有着ConcurrentHashMap中极为重要的几个属性（SIZECTL）
        // ，unsafe静态块控制其修改行为。Java8中，大量运用CAS进行变量、
        // 属性的无锁修改，大大提高性能。
    // Unsafe mechanics
    private static final sun.misc.Unsafe U;
